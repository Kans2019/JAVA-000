...
public class Response {
    ...
    public Response(FullHttpResponse httpResponse) {
        this.statusCode = httpResponse.status().code();
        this.reasonPhrase = httpResponse.status().reasonPhrase();
        this.content = Objects.isNull(httpResponse.content()) ? new byte[0] : httpResponse.content().toString(Charset.defaultCharset()).getBytes();
